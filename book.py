...
def create_book_for_user(book: BookCreate,  db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def update_book_for_user(book_id: int, book: BookUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def delete_book_for_user(book_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
